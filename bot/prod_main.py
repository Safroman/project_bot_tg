...
def show_payment(call):
    ...
    if step == BACK_IND:
        ...
        def process_signals_payment(message, package, user_id):
            user = Users.get_user(user_id=str(message.chat.id))
            txid = message.text
            if txid in ABORT_BUTTON.values():
                ...
                return
            elif (len(txid) == 64 and txid.isalnum()) or (len(txid) == 10 and txid.isdigit()):
                ...
                msg = bot.send_message(message.chat.id, INVALID_TXID[user.lang] + ABORT_BUTTON[user.lang],
                                       reply_markup=kb)
                bot.register_next_step_handler(msg, process_signals_payment, package, user_id)

        def process_following_payment(message, package, user_id):
            user = Users.get_user(user_id=str(message.chat.id))
            txid = message.text
            if txid in ABORT_BUTTON.values():
                kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
                ...
                msg = bot.send_message(message.chat.id, INVALID_TXID[user.lang] + ABORT_BUTTON[user.lang],
                                       reply_markup=kb)
                bot.register_next_step_handler(msg, process_following_payment,
                                               package=package,
                                               user_id=user_id)

        msg = bot.send_message(call.message.chat.id, f'{TXID_REQUEST[user.lang]}')

        if package in SIGNALS_PACKAGES_KB.keys():
            bot.register_next_step_handler(msg, process_signals_payment,
                                           package=call.data.split(SP)[-2],
